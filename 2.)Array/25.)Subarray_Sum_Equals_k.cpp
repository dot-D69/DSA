...
// The problem is to find the number of subarrays having sum exactly equal to k.
// example: arr[] = {10, 2, -2, -20, 10}, k = -10
// Output : 3
// explanation: arr[0...3], arr[1...4], arr[3..4] have sum exactly equal to k.
...
int main()
{
    ...
    int k = -10;
    cout << countSubarraysWithSumK(a, k) << endl;
    return 0;
}
//  however this approach will not work for negative numbers in the array and may result in TLE for large inputs.
...
we use rest to see if we have reached our target
sum or if we need to remove or add values
...
we can only find this if we subtrac the 3 from 9
 (9-3 =6) now we need to subtract the subarray of
 sum 6 from our array ( of sum 9) to get a value
 of  3 sum subarray

now we check if there is any subarray of sum=6 in
 the map if there is then we get the count of the
  times it is present if it is preset two times
  so it means  that in the current array we have
   two such subarrays where sum =3 (cause we will
  minus the 6 sum subarray two times)
